"""
Converter Service - Handle PDF conversion to Word, Excel, PowerPoint and vice versa
"""
import os
import requests
...
def convert_local(pdf_file, docx_file):
    """Convert PDF to DOCX using local library (pdf2docx)"""
    try:
...
def convert_cloud(pdf_file, docx_file):
    """Convert PDF to DOCX using ConvertAPI (Cloud)"""
    try:
        if not MY_API_KEY:
...
def convert_cloud_generic(pdf_file, output_file, output_format):
    """Convert PDF to other formats using ConvertAPI (xlsx, pptx)"""
    try:
        if not MY_API_KEY:
            return {"status": False, "message": "Lỗi bảo mật: Chưa cấu hình API Key trong file .env"}
...
def pdf_to_excel(pdf_file, xlsx_file, mode='local'):
    ...
    # Excel only supports cloud mode (ConvertAPI)
